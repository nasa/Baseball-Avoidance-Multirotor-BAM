...
# error "Classic call interface is not supported by rt_malloc_main.cpp."
#endif

#if ONESTEPFCN==0
#error Separate output and update functions are not supported by rt_malloc_main.cpp. \
You must update rt_malloc_main.cpp to suit your application needs, or select \
the 'Single output/update function' option.
#endif

#if TERMFCN==0
#error The terminate function is required by rt_malloc_main.cpp. \
You must update rt_malloc_main.cpp to suit your application needs, or select \
the 'Terminate function required' option.
#endif

#if ALLOCATIONFCN==0 
# error An allocation function is required by rt_malloc_main.cpp. \
You must update rt_malloc_main.cpp to allocate model instance data, or select \
...
#error The static version of rt_malloc_main.cpp does not support model step function prototype control.
#endif

#if ROOT_IO_FORMAT != 2
# error rt_malloc_main.cpp requires root-level I/O to be passed as part of model \
data structure.                                                                \
You must update rt_malloc_main.cpp  to suit your application needs, or set        \
...
int_T main(int_T argc, const char *argv[])
{
    ...
    // TCB:  modified logic to allow passing in optional MATFILE name
    if (argc > 1)
      rt_StopDataLogging(argv[1], rtmGetRTWLogInfo(S));
    else
