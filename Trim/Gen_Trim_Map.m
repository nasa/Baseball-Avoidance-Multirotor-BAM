function [t_struc] = Gen_Trim_Map(tmap_fname)
...
% and to perform this repeatedly to generate a trim map.  This trim map is 
% generated using the outdated aero-propulsive model
%
% Inputs
...
lb = [-pi/2; -pi/2; 0; 0; 0; 0];
ub = [pi/2; pi/2; inf; inf; inf; inf];
prop_0 = 4000*ones(4,1); % Initial prop rotor estimate
...
for u_loop = u_vec
    ...
    for v_loop = v_vec
        ...
        for w_loop = w_vec
            ...
            eu_0 = [x(1:2);prop_0];
            [ eu, fval, exitflag, output, ~, ~, ~ ] = fmincon(@(eu)mycost(x(3:12), eu, SimIn.c, g_fps2, rho, aero_params, SimIn.Units),...
                eu_0, [], [], [], [],lb, ub);

            t_struc.phi(v_cnt, u_cnt,w_cnt) = eu(1);
            t_struc.theta(v_cnt, u_cnt,w_cnt) = eu(2);
            t_struc.p1(v_cnt, u_cnt,w_cnt) = eu(3);
            t_struc.p2(v_cnt, u_cnt,w_cnt) = eu(4);
            t_struc.p3(v_cnt, u_cnt,w_cnt) = eu(5);
            t_struc.p4(v_cnt, u_cnt,w_cnt) = eu(6);
