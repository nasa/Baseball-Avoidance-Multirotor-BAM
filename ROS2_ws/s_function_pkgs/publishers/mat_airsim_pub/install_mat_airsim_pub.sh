#!/bin/bash
#
# install_mat_airsim_pub.sh
#
# This script installs the updated mat_airsim_pub package files into a ROS workspace.
#
# Expected Git repository structure (to be run from within this directory):
#
#  mat_airsim_pub/
#    ├── CMakeLists.txt
#    ├── install_mat_airsim_pub.sh   <-- (this script)
#    ├── package.xml
#    └── src
#         ├── AirSim_Mat_Main.cpp
#         └── ros_msg_sim.cpp
...
#
# Usage:
#   ./install_mat_airsim_pub.sh [TARGET_WORKSPACE_DIRECTORY]
#
set -e
...
# Set package name (in this instance: mat_airsim_pub) and target package directory.
PKG_NAME="mat_airsim_pub"
...
if [ ! -d "$TARGET_PKG_DIR" ]; then
    ...
    ros2 pkg create --build-type ament_cmake ${PKG_NAME} --node-name ${PKG_NAME}_node \
...
4. If you have't set the PATH variable to include ${TARGET_WS}/build/mat_airsim_pub, do so. Otherwise, from MATLAB, run the following prior to simulation setup:
   addpath(genpath('${TARGET_WS}/build/mat_airsim_pub'))
EOF
