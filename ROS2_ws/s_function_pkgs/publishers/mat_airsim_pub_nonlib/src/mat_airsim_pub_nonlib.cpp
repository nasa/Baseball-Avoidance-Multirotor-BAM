...
#define S_FUNCTION_NAME mat_airsim_pub_nonlib
...
{
public:
  PubPoseNode() : Node("pose_pub_node") // MODIFY NODE NAME
    {
      publisher_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("pub_pose",10); //TOPIC NAME HERE
    ...
    msg.pose.orientation.x = quat[0]; // Note that the quat w component is the 
    msg.pose.orientation.y = quat[1]; // last element which agrees with the 
    msg.pose.orientation.z = quat[2]; // ROS2 msg PoseStamped
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if (DEBUG_FLAG){std::cout << "Pub Node is being created..\n";}
    ssGetPWork(S)[0] = (void *)  new PubPoseNode; // creates the new object
...
{
    PubPoseNode *node = (PubPoseNode *) ssGetPWork(S)[0];
...
static void mdlTerminate(SimStruct *S){
    ...
    PubPoseNode *node = static_cast<PubPoseNode *>(ssGetPWorkValue(S, 0));
